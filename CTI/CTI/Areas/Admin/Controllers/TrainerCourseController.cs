...
namespace CTI.Areas.Admin.Controllers
{
    ...
    public class TrainerCourseController : Controller
    {
        ...
        public TrainerCourseController(ApplicationDbContext context, IWebHostEnvironment hostingEnvironment
        ...
        public async Task<IActionResult> Add()
        {
            try
            {
                ...
                if (files.Count > 0)
                {
                    ...
                    var uploads = Path.Combine(webRootPath, @"TrainerCoursesFiles\");
                    ...
                    using (var package = new ExcelPackage(new FileInfo(Path.Combine(uploads, uniqueFileName))))
                    {
                        ...
                        for (int row = 2; row <= worksheet.Dimension.End.Row; row++)
                        {
                            ...
                            var Trainer = await _context.ApplicationUsers.FirstOrDefaultAsync(x => x.UserName == trainingNumber);


                            var selectedTraineeCourses = await _context.ApplicationUserCourses
                                      .Where(ptu => ptu.CourseId == course.Id && ptu.UserId == Trainer.Id)
                                      .FirstOrDefaultAsync();
        ...
        public async Task<IActionResult> DownloadExcel()
        {
            try
            {
                string FileName = "مثال اضافة المقررات للمدربين.xlsx";
